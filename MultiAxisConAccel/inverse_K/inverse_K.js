...
let rot_base_speed = 0.2;  //base speed of change in rotation
let tracker = [false,false,false]; //tracking array
...
function setup() {
  ...
  //set path queue
  path.push(createVector(10,100));  //start at home
  path.push(createVector(20,50)); 
  path.push(createVector(50,80));// desired end point
...
function draw() {
  ...
  label_diagram(l1.x,l1.y,l2.end_x,l2.end_y,a,b,l1.L_len,l2.L_len);
    ...
    tracker[0] = move_angles(path[0],path[2]);
...
function calculate_ik(dest_x,dest_y){
...
function move_angles(src,dest){
  ...
  var p1_angles = calculate_ik(src.x,src.y);
  var p2_angles = calculate_ik(dest.x,dest.y);
...
//function to write home position/angles to joints and reset tracker
function home(home_point){
  var buff = calculate_ik(home_point.x,home_point.y);
  a = buff.x;
  b = buff.y;
...
function label_diagram(origin_x,origin_y,end_x,end_y,alpha,beta,L1_len , L2_len){
  ...
  text('Inputs:\nP1(' + path[0].x + ','+ path[0].y+')\nP2(' + path[1].x + ',' + path[1].y + ')\nP3(' + path[2].x + ',' + path[2].y + ')',150,80);
