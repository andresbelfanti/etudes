...
#define MPU_addr0 0x68  // default MPU6050 address
#define MPU_addr1 0x69  // default MPU6050 address
//-----------------------------------------------------------
float acReadings0[3][AC_NUM_TO_AVG + 1];  // [x,y,z],[readings,sum]
int acAvgIndex0 = 0;  // track which index of array is next to be updated
float acInst0[3] = {0,0,0};  // x,y,z
float acAvg0[3] = {0,0,0};  // x,y,z
float acPR0[2] = {0,0};  // accel pitch and roll
float gyDeltaPRY0[3] = {0,0,0};  // gyro pitch roll and yaw
float acGyOffset0[6] = {-247.69, -133.50, -68.48, 905.85, -151.32, -110.68};  // accel: x,y,z and gyro: x,y,z offsets - paste from calibration results
float gyAcMix0 = 0.95;  // adjust mix of gyro and accel in data combination, 0 = all accel, 1 = all gyro
float pry0[3] = {0,0,0};  // final pitch roll and yaw

float acReadings1[3][AC_NUM_TO_AVG + 1];  // [x,y,z],[readings,sum]
int acAvgIndex1 = 0;  // track which index of array is next to be updated
float acInst1[3] = {0,0,0};  // x,y,z
float acAvg1[3] = {0,0,0};  // x,y,z
float acPR1[2] = {0,0};  // accel pitch and roll
float gyDeltaPRY1[3] = {0,0,0};  // gyro pitch roll and yaw
float acGyOffset1[6] = {-247.69, -133.50, -68.48, 905.85, -151.32, -110.68};  // accel: x,y,z and gyro: x,y,z offsets - paste from calibration results
float gyAcMix1 = 0.95;  // adjust mix of gyro and accel in data combination, 0 = all accel, 1 = all gyro
float pry1[3] = {0,0,0};  // final pitch roll and yaw
...
void updateAcReadings(){
  ...
  Wire.beginTransmission(MPU_addr0);
  Wire.write(0x3B);
  Wire.endTransmission(false);
  Wire.requestFrom(MPU_addr0, 6, true);
  int16_t xAcRaw = (Wire.read() << 8 | Wire.read());
  int16_t yAcRaw = (Wire.read() << 8 | Wire.read());
  int16_t zAcRaw = (Wire.read() << 8 | Wire.read());


  acInst0[0] = (float)xAcRaw + acGyOffset[0];
  acInst0[1] = (float)yAcRaw + acGyOffset[1];
  acInst0[2] = (float)zAcRaw + acGyOffset[2];

  Wire.beginTransmission(MPU_addr1);
  Wire.write(0x3B);
  Wire.endTransmission(false);
  Wire.requestFrom(MPU_addr1, 6, true);
    xAcRaw = (Wire.read() << 8 | Wire.read());
   yAcRaw = (Wire.read() << 8 | Wire.read());
   zAcRaw = (Wire.read() << 8 | Wire.read());

  acInst1[0] = (float)xAcRaw + acGyOffset[0];
  acInst1[1] = (float)yAcRaw + acGyOffset[1];
  acInst1[2] = (float)zAcRaw + acGyOffset[2];
